...
export default function Create(props: any) {
  ...
  const [isBSPApproved, setIsBSPApproved] = useState(false)
  ...
  const getContractStates = async () => {
    ...
    if (Number(allowance) >= Number(price)) {
      setIsBSPApproved(true)
    } else {
      setIsBSPApproved(false)
    }
  }
                  ...
                  {isBSPApproved ? (
                    <Stack direction="row" align="flex-start" spacing="0rem" flexWrap="wrap" shouldWrapChildren>
                      <Box maxW="sm" borderWidth="1px" p={1} rounded="lg" overflow="hidden">
                        <Text>Creating a vault spends {price * Math.pow(10, -decimals)} BSP from your wallet</Text>
                      </Box>
                    </Stack>
                  ) : null}
                  ...
                  {Number(balance) < Number(price) ? (
                    <Box d="flex" alignItems="baseline" justifyContent="space-between" mt="2">
                      <Button
                        ...
                        // }}
                        onClick={()=>{initializeTransak(vaultAddress, 'BSP')}}
                      >
                        Buy $BSP
                      </Button>
                    </Box>
