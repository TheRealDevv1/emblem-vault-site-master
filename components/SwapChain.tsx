...
export default function SwapChain() {
  ...
  const [BSPApprovedFor, setIsBSPApproved] = useState(0)
  const [hash, setHash] = useState(null)
  const [swapAmount, setSwapAmount] = useState(0)
  const [isInvalid, setIsInvalid] = useState(false)
  const [transferChainChanged, setTransferChainChanged] = useState(true)
  const handlerContract = useContract(contractAddresses.vaultHandler[chainId], contractAddresses.vaultHandlerAbi, true)
  const BSPContract = useContract(contractAddresses.BSP[chainId], contractAddresses.BSPAbi, true)
  ...
  const getContractStates = async () => {
    setHash(null)
    setDecimals(await BSPContract.decimals())
    setAllowance(
      await BSPContract
        .allowance(account, contractAddresses.vaultHandler[chainId])
        .then((balance: { toString: () => string }) => balance.toString())
    )
    setBalance(await BSPContract.balanceOf(account).then((balance: { toString: () => string }) => balance.toString()))
    if (Number(allowance) >= Number(0)) {      
      setIsBSPApproved(allowance)
    }
    setState({ loaded: true })
  }

  const approveBSPFlow = () => {
    setApproving(true)
    ;(BSPContract as Contract)
        ...
        <Stack align="center">
            ...
            <>
              {/* <Box mt="2" ml="4" lineHeight="tight">
                <Text mt={2} as="h4" ml="4" mr="4" fontSize="s" fontStyle="" >
                  Transfer up to {allowance > balance ? balance * Math.pow(10, -decimals) : allowance * Math.pow(10, -decimals) } BSP from {chainId == 137 ? "Matic" : "Ethereum"} Network to {chainId == 137 ? "Etherum" : "Matic"} Network
                </Text>              
              </Box>
              {balance > allowance ? (                 
                <Box mt="2" ml="4" lineHeight="tight">
                  <Button onClick={approveBSPFlow} type="submit">
                    Increase transfer allowance to { balance * Math.pow(10, -decimals) } BSP
                  </Button>
                </Box>
              ) : null } */}
          ...
          allowance == 0 && balance > 0 && !approving ? (
            <Box mt="2" ml="4" lineHeight="tight">
              <Text mt={2} as="h4" ml="4" mr="4" fontSize="s" fontStyle="italic" >
                <Button onClick={approveBSPFlow} type="submit">
                  Approve Transfer{/* of up {balance * Math.pow(10, -decimals)} BSP*/}
                </Button>
              </Text>
            </Box>
          ) : balance == 0 ? (
            <Box mt="2" ml="4" lineHeight="tight">
              <Text mt={2} as="h4" ml="4" mr="4" fontSize="s" fontStyle="italic" >
                You have no BSP to transfer
              </Text>
            </Box>
          ) : !approving ? (
            <Box mt="2" ml="4" lineHeight="tight">
              <Text mt={2} as="h4" ml="4" mr="4" fontSize="s" fontStyle="italic" >
                Approved to transfer up to {allowance * Math.pow(10, -decimals)} BSP. Note: Swaps between networks can take up to 25 minutes.
              </Text>
            </Box>
          ) : null }
          {/* { !approving && ((Number(balance) > Number(allowance)) || Number(allowance) * Math.pow(10, -decimals) < 1000000) ? (
            <Box mt="2" ml="4" lineHeight="tight">
              <Text mt={2} as="h4" ml="4" mr="4" fontSize="s" fontStyle="italic" >
              <Button onClick={approveBSPFlow} type="submit">
                  Approve Transfer 1,000,000 BSP
                </Button>
              </Text>
            </Box>
          ) : null } */}
